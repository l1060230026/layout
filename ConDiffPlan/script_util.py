...
def update_arg_parser(args):
    ...
    if not hasattr(args, 'rtype_dim'):
        args.rtype_dim = 25
    if not hasattr(args, 'corner_index_dim'):
        args.corner_index_dim = 32
    if not hasattr(args, 'room_index_dim'):
        args.room_index_dim = 32
    
    # Calculate condition_channels dynamically based on hyperparameters
    calculated_condition_channels = args.rtype_dim + args.corner_index_dim + args.room_index_dim
    
    if args.dataset=='rplan':
        args.input_channels = num_coords * 1 # . , . , . , . , '
        args.condition_channels = calculated_condition_channels + 1  # +1 for rplan specific
        args.out_channels = num_coords * 1
        args.use_unet = False

    elif args.dataset=='msd':
        args.input_channels = num_coords * 1 # . , . , . , . , '
        args.condition_channels = calculated_condition_channels
        args.out_channels = num_coords * 1
        args.use_unet = False

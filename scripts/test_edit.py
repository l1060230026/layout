...
class RPlanhgDataset():
    def __init__(self, length=None, image_id=None, non_manhattan=False):
        ...
        self.cnumber_dist = np.load(f'processed_rplan/rplan_train_8_cndist.npz', allow_pickle=True)['cnumber_dist'].item()
        ...
        for index, (h, graph) in tqdm(enumerate(zip(self.org_houses, self.org_graphs)), total=len(self.org_houses), desc='processing dataset'):
            ...
            include_ids.append(index)
        
        self.org_houses = [self.org_houses[i] for i in include_ids]
        self.org_graphs = [self.org_graphs[i] for i in include_ids]
        
    def add_room(self, house_idx, new_room_type, connect_to_room_idx):
        """
        Add a new room to an existing floorplan and connect it to a specified room.
        
        Args:
            house_idx (int): Index of the house to modify
            new_room_type (int): Type of the new room to add
            connect_to_room_idx (int): Index of the room to connect the new room to

        Returns:
            bool: True if room was successfully added, False otherwise
        """
        
        connect_to_room_idx -= 1
            
        # Convert new room corners to the same format as existing rooms
        num_room_corners = random.choice(self.cnumber_dist[new_room_type])
        new_room_corners = np.zeros([num_room_corners, 2])
        num_door_corners = random.choice(self.cnumber_dist[17])
        new_door_corners = np.zeros([num_door_corners, 2])
        
        # Get current house data
        nodes = [self.org_houses[house_idx][i][1] for i in range(len(self.org_houses[house_idx]))]
        room_end = nodes.index(17)
        self.org_houses[house_idx].insert(room_end, [new_room_corners, new_room_type])
        
        nodes = [self.org_houses[house_idx][i][1] for i in range(len(self.org_houses[house_idx]))]
        door_end = nodes.index(15) - 1
        self.org_houses[house_idx].insert(door_end, [new_door_corners, 17])
        
        # Update self.org_graphs to reflect the new room and door
        # Get the current graph
        current_graph = self.org_graphs[house_idx].copy()
        
        # Update indices in current_graph
        # All indices after room_end need to be incremented by 1 (for new room)
        # All indices after door_end need to be incremented by 1 (for new door)
        for i in range(len(current_graph)):
            if current_graph[i][0] >= room_end:
                current_graph[i][0] += 1
            if current_graph[i][2] >= room_end:
                current_graph[i][2] += 1
                
            if current_graph[i][0] >= door_end:
                current_graph[i][0] += 1
            if current_graph[i][2] >= door_end:
                current_graph[i][2] += 1
        
        # Add connection between new room and the room it's connecting to
        new_room_idx = room_end
        new_door_idx = door_end
        new_connections = [
            [new_room_idx, 1, connect_to_room_idx],  # Connect new room to specified room
            [new_door_idx, 1, new_room_idx],         # Connect door to new room
            [new_door_idx, 1, connect_to_room_idx]   # Connect door to specified room
        ]
        
        # Add non-connections to all other rooms
        for i in range(len(self.org_houses[house_idx])):
            if i != new_room_idx and i != new_door_idx and i != connect_to_room_idx:
                new_connections.append([new_room_idx, -1, i])
                new_connections.append([new_door_idx, -1, i])
        
        # Update the graph with new connections
        self.org_graphs[house_idx] = np.concatenate((current_graph, np.array(new_connections)), 0)
        
        h, graph = self.org_houses[house_idx], self.org_graphs[house_idx]
        house = []
        corner_bounds = []
        num_points = 0
        for i, room in enumerate(h):
        ...
        house_layouts = np.concatenate(house, 0)
        if len(house_layouts)>self.max_num_points:
            return False
        padding = np.zeros((self.max_num_points-len(house_layouts), 94))
        gen_mask = np.ones((self.max_num_points, self.max_num_points))
        gen_mask[:len(house_layouts), :len(house_layouts)] = 0
        house_layouts = np.concatenate((house_layouts, padding), 0)

        door_mask = np.ones((self.max_num_points, self.max_num_points)) # Connected rooms
        self_mask = np.ones((self.max_num_points, self.max_num_points)) # Within the same room
        for i in range(len(corner_bounds)):
            for j in range(len(corner_bounds)):
                if i==j:
                    self_mask[corner_bounds[i][0]:corner_bounds[i][1],corner_bounds[j][0]:corner_bounds[j][1]] = 0
                    ...
                    door_mask[corner_bounds[i][0]:corner_bounds[i][1],corner_bounds[j][0]:corner_bounds[j][1]] = 0
        self.houses[house_idx] = house_layouts
        self.door_masks[house_idx] = door_mask
        self.self_masks[house_idx] = self_mask
        self.gen_masks[house_idx] = gen_mask
        self.graphs[house_idx] = graph
...
def main():
    ...
    dataset = RPlanhgDataset(image_id='21733')
    ...
    data_samples = []
    data_sample, model_kwarg = dataset.get_data(process_house_idx)
    ...
    graph_error = save_samples(sample_gt, 'gt', model_kwargs, tmp_count, num_room_types, ID_COLOR=ID_COLOR, is_syn=True, draw_graph=args.draw_graph, save_svg=args.save_svg, room_class=ROOM_CLASS, show_room_names=True)
    graph_error = save_samples(sample, 'pred', model_kwargs, tmp_count, num_room_types, ID_COLOR=ID_COLOR, is_syn=True, draw_graph=args.draw_graph, save_svg=args.save_svg, save_gif=False, room_class=ROOM_CLASS, show_room_names=True)


    assert dataset.add_room(house_idx=process_house_idx, new_room_type=2, connect_to_room_idx=3)
    model_kwargs = {}
    data_samples = []
    data_sample, model_kwarg = dataset.get_data(process_house_idx)
    data_samples.append(data_sample)
    ...
    tmp_count += 1
    graph_error = save_samples(sample, 'pred', model_kwargs, tmp_count, num_room_types, 
...
def create_argparser():
    defaults = dict(
        ...
        batch_size=16,
        use_ddim=False,
        model_path="ckpts/rplan_8/model500000.pt",
        draw_graph=True,
        save_svg=True,
    )
